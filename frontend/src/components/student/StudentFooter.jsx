...
const navLinks = [
  { id: 'student-dashboard', label: 'Dashboard', action: 'dashboard' },
  { id: 'student-homes', label: 'Rental Homes', action: 'homes' },
  { id: 'student-hostels', label: 'Hostels', action: 'hostels' },
...
const StudentFooter = ({ onNavigate }) => {
  ...
  return (
    <footer className="mt-8 border-t border-[#d0d8ff] bg-[linear-gradient(118deg,#0a0d3c_0%,#101a57_42%,#1e2f83_100%)] text-white shadow-[0_-18px_36px_-28px_rgba(8,14,56,0.95)]">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-7 sm:pt-9 pb-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-7">
          <div>
            ...
            <p className="mt-3 text-sm text-white/78 leading-relaxed max-w-[28ch]">
              Student dashboard for discovering verified homes and hostels near DIU.
            </p>
          </div>
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-white/65 font-semibold">Student Portal</p>
            <p className="mt-3 text-sm text-white/85 leading-relaxed">Designed for DIU students to browse nearby housing options with confidence.</p>
            <p className="mt-2 text-xs text-white/70">Version 1.0 Student Dashboard</p>
        ...
        <div className="mt-6 sm:mt-8 pt-3 border-t border-white/15 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[11px] sm:text-xs text-white/70">
          ...
          <p>Built for DIU student housing discovery.</p>
        </div>
      </div>
    </footer>
  )
