...
const OwnerProfilePage = ({ owner, onSaveProfile }) => {
  const initialValues = useMemo(
    () => ({
      fullName: owner?.fullName || owner?.name || '',
      email: owner?.email || '',
      phone: owner?.phone || '',
      buildingName: owner?.buildingName || owner?.building_name || '',
      location: owner?.location || '',
      nationalId: owner?.nationalId || owner?.national_id || owner?.diu_student_id || '',
      bio: owner?.bio || '',
  ...
  const roleLabel = owner?.role
    ? owner.role.replace('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase())
    : 'Owner'

  const initials = (formData.fullName || 'Owner')
    .trim()
    .split(/\s+/)
    .slice(0, 2)
    ...
    <div className="space-y-6 sm:space-y-7">
      <section className="rounded-[22px] border border-[#9bb0ff] bg-[linear-gradient(132deg,#18205c_0%,#23358f_48%,#3650be_100%)] px-5 sm:px-7 py-6 text-white shadow-[0_24px_45px_-30px_rgba(14,27,99,0.95)]">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.12em] text-white/70 font-semibold">Owner Portal</p>
            ...
            <p className="mt-2 text-sm text-white/85 max-w-2xl">
              Keep your account details up to date so students can trust your listings and contact information.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.65fr_1fr] gap-5 sm:gap-6">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Field
              ...
              value={formData.bio}
              onChange={handleChange}
              placeholder="Tell students about your property and management style"
              as="textarea"
              className="md:col-span-2"
            />
          </div>
        ...
        <aside className="rounded-[22px] border border-[#c8d3ff] bg-white p-5 sm:p-6 shadow-[0_20px_38px_-32px_rgba(36,56,143,0.65)]">
          ...
          <div className="mt-4 rounded-2xl border border-[#d9e2ff] bg-[linear-gradient(165deg,#f4f7ff_0%,#ffffff_100%)] p-4">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-full overflow-hidden ring-2 ring-[#d4defe] bg-[#dce5ff] flex items-center justify-center text-[#2f4eca] font-semibold">
                {formData.avatarUrl ? (
                  <img src={formData.avatarUrl} alt={formData.fullName || 'Owner'} className="w-full h-full object-cover" />
                ) : (
                  initials || 'O'
                )}
              </div>
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-[#25305e] truncate">{formData.fullName || 'Owner Name'}</p>
                <p className="text-xs text-[#7480b2] truncate">{formData.email || 'owner@email.com'}</p>
              </div>
            </div>

            <div className="mt-4 space-y-2 text-xs text-[#4b578e]">
              <p><span className="font-semibold">Phone:</span> {formData.phone || '-'}</p>
              <p><span className="font-semibold">Building:</span> {formData.buildingName || '-'}</p>
          ...
          <div className="mt-4 rounded-2xl border border-[#dce4ff] bg-[#f8faff] p-3.5 text-xs text-[#6471a7] leading-relaxed">
            Your name, email, and phone update your account profile. Extra details like bio, building, and location improve trust across your owner dashboard.
          </div>
        </aside>
      </div>
    </div>
